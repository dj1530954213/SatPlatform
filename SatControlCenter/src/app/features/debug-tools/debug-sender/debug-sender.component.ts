...
export class DebugSenderComponent {
  ...
  debugNote: string = 'Test note from ControlCenter UI @ ' + new Date().toLocaleTimeString();
  customSharedDataString: string = '{}';
  ...
  async sendNote() {
    ...
    try {
      ...
      this.apiResponse = await this.debugService.sendDebugNote(this.groupId, this.debugNote, this.customSharedDataString);
    } catch (error) {
